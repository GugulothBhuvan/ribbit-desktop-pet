...
| Voice / PTT | Phase 5 roadmap, PTT hotkey | Deepgram PTT | Spacebar PTT | **Global `Ctrl+Shift+Space` toggle (RegisterHotKey)** | A focused-window Space handler contradicts PRD 8.6 (never take focus) and swallows the user's spacebar. Bare Space cannot be a global hotkey. |
